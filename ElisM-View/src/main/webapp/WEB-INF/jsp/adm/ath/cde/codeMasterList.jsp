		...
		<div class="search-table-wrapper">
			<div class="form-group form-control-multiple">
				...
				<select class="form-control input-sm" name="searchCondition" id="searchCondition">
					<option value="CODE_ID" <c:if test="${empty searchDefaultVO.searchCondition || searchDefaultVO.searchCondition == 'CODE_ID'}">selected="selected"</c:if>><spring:message code="button.codeId" /></option>
    	...
    	firstGrid.setConfig({
            ...
            body: {
                ...
                onClick: function () {
                	...
                	fnModalOpen("url", "primary", "360", "160", "10000", "코드 상세", "/adm/ath/cde/getCodeMaster.do", paramData);
                }
            },
            page: {
...
function codeInfoListToJson(_pageNo){
	$.ajax({
		type : "POST",
		url : "/adm/ath/cde/getCodeMasterListJson.do",
		dataType : "json",
		...
		success : function(data) {
	        ...
	        for (var i = 0, l = resultList.length; i < l; i++) {
			    list.push({codeId: resultList[i].codeId, codeNm: resultList[i].codeNm, createId: resultList[i].createId, createDt: resultList[i].createDt});
			}
	        firstGrid.setData({
	            list: list,
	            page: {
...
function fnUserInfoInsert(){
	fnModalOpen("url", "primary", "360", "160", "10000", "코드 등록", "/adm/ath/cde/viewCodeMasterInsert.do", "");
}
