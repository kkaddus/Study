...
function fnUpdate(){
	...
	if(valCheck){
		...
		$.ajax({
			type : "POST",
			url : "/adm/ath/cde/updateCodeDetailJson.do",
			dataType : "json",
			data : paramData,
...
function fnDelete(){
	...
	if(valCheck){
		...
		$.ajax({
			type : "POST",
			url : "/adm/ath/cde/deleteCodeDetailJson.do",
			dataType : "json",
			data : paramData,
			success : function(data, textStatus, XMLHttpRequest) {
				alert(data.resultMessage);
	...
	<table class="input-table table table-bordered">
		...
		<tbody>
			<tr>
				...
				<td>
					<input type="text" size="20" maxlength="10" disabled id="codeId" name="codeId" class="form-control input-sm validationCheck" value="${codeDetail.codeId}"/>
				</td>
			</tr>
			<tr>
				<th>코드상세번호<img src="<c:url value='/'/>images/required.gif" alt="필수항목" title="필수항목" width="15" height="15" /></th>
				<td>
					<input type="text" size="20" maxlength="10" disabled id="codeDesc" name="codeDesc" class="form-control input-sm validationCheck" value="${codeDetail.codeDesc}"/>
				</td>
			</tr>
			<tr>
				<th>코드이름<img src="<c:url value='/'/>images/required.gif" alt="필수항목" title="필수항목"  /></th>
				<td>
					<input type="text" size="20" maxlength="60" id="codeNm" name="codeNm" class="form-control input-sm validationCheck" value="${codeDetail.codeNm}">
				</td>
			</tr>
		</tbody>
	</table>
