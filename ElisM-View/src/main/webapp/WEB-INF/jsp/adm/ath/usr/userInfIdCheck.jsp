...
function fnCheckId(){
    ...
    if(fnCheckNotKorean($("#checkId").val())){
    	$.ajax({
    		type : "POST",
    		url : "/adm/ath/usr/userInfIdCheckJson.do",
    		dataType : "json",
    		...
    		success : function(data) {
    			...
    			if(data.usedCnt == "0"){
        	        $("#resultdiv").html(data.checkId + "는 사용가능한 아이디입니다.");
    			}else if(data.usedCnt == "-1"){
        	        $("#resultdiv").html("중복확인을 실행하십시오");
    			}else{
        	        $("#resultdiv").html(data.checkId + "는 사용할수 없는 아이디입니다.");
    			}
    		}
    	});	
    }else{
...
function fnReturnId(){
    if ($("#usedCnt").val() == "0"){
        $("#userId", parent.document).val($("#resultId").val());
...
<form name="checkForm" id="checkForm" onsubmit="return false;" style="padding: 5px; text-align: center; overflow:auto;">
	...
	<div class="text-center">
		<a href="#" onclick="fnCheckId();" class="btn btn-grey">조회</a>
		<a href="#" onclick="fnReturnId();" class="btn btn-primary">사용</a>
